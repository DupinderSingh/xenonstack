	...
	function renderInlineUI(args) {
		...
		function show() {
			if (panel) {
				panel.show();
				reposition();
				DOM.addClass(editor.getBody(), 'mce-manage-focus');
			}
		}
		...
		function hide() {
			if (panel) {
				...
				DOM.removeClass(editor.getBody(), 'mce-manage-focus');
			}
		}
	...
	function renderIframeUI(args) {
		...
		panel = self.panel = Factory.create({
			...
			items: [
				settings.menubar === false ? null : {type: 'menubar', border: '0 0 1 0', items: createMenuButtons()},
				createToolbars(settings.toolbar_items_size),
				{type: 'panel', name: 'iframe', layout: 'stack', classes: 'manage-area', html: '', border: '1 0 0 0'}
			]
		});
